...
- **Review-chain tiers and Workflow orchestration (Claude side, 0.13.0) are Claude-only**: Codex subagents have no per-stage model/effort knob and no Workflow `agent()`/`parallel()`, so on the Codex side "blind start + chained digging" keeps only the orchestration rules (2 read-only review subagents run blind in parallel and the main conversation merges without counting; dig rounds carry every earlier finding, refute first, then fill gaps on a new lens; stop when the blind round has nothing at P0/P1 and no contradictions, a contradiction alone is settled by the main conversation, a P0/P1 is fixed and then chained until the main conversation rules nothing new at P0/P1, cap 4 rounds); the verdict doesn't escalate tiers and the main conversation rules — see AGENTS.md §7.1.
...
| `docs/Progress.md` | Module status overview table + changelog (newest first; changelog keeps only the current month, older entries are archived monthly to `docs/archive/`) |
...
| **Frontend artist** | `opus` subagent | Frontend visual/motion/UI polish: animation feel, layout look, bubble/overlay positioning, color and spacing — the "does it look right" kind of work (empirically, opus at medium tier beats sonnet here; prompt guidance in 7.4) |
...
| Blind round of a review chain (first round: 2–3 agents, none seeing the others, parallel, no vote counting) and dig rounds (carry every finding; refute first, then fill gaps); principle-3 units run every round at high | `opus` | `medium` |
...
2. **`xhigh` / `max` never enter batch stages.** Reserve them for the rare single-point decision: the hardest one-shot verdict, a security audit — judgment calls where a mistake is expensive. If a review wants more confidence, **add rounds** — don't escalate a single round to xhigh, and don't count votes. Review is always **blind start + chained digging + verdict**: **Blind round** — the first round dispatches 2 review agents (3 for the high-risk units of principle 3) in parallel, none seeing the others, each discovering from scratch on a fixed lens: correctness; boundary and malformed data (the third adds security); checking the tests themselves is a baseline check every review agent does and takes no lens slot. **No vote counting**: every agent returns the same schema, `findings[{file, line, severity: P0|P1|P2, claim, lens}]`; the script merges by file+line (repo-relative path, start line, ±3 lines count as the same spot; no line number → match on file + the first 40 chars of the claim; what can't be normalised isn't merged, the next round dedups it) and marks entries at the same location with opposite conclusions or different severities as `conflict`; semantic contradictions the script can't judge go to the next round. **Dig rounds** — every later round is one agent whose prompt explicitly carries every earlier finding (confirmed items, overturned items, checkpoints already re-verified); its job is to try to refute each one first, then find what was missed, on a lens not used before (in order: security, spec acceptance-criteria coverage, integration seams, rollback and reversibility, adversarial refutation), reporting only new findings and reversals. **Verdict round** — the last round of the chain (`opus` + `high`, ruling on the whole chain's findings, no lens slot); blind and dig rounds are `opus` + `medium`. **Routing** (after the blind merge): only P2 → pass, P2s recorded as leftovers; nothing at P0/P1 but a conflict → no fix, go straight to a verdict round to settle it; P1 → fix, then a verdict round; P0 → fix, then one dig round, then the verdict round; a verdict round that reports new P0/P1 → fix and route the same way, counting toward the cap. Stop when the verdict round reports no new P0/P1 and nothing at P0/P1 is left open (closed = fixed and confirmed by a later round; new P2s are just recorded, no extra round). Default cap 4 rounds (the blind round counts as one), going past it needs a stated reason. **Fixing and resuming**: fixes never happen inside the review stage — as soon as a P0/P1 needs fixing, this run is over for that unit; the main conversation dispatches implementation agents through the rejection flow (tests and implementation as two separate agents) and then starts a new run that continues the same chain; the chain's state lives in the repo at `docs/reviews/<unit-or-spec>-chain.md` (created by the main conversation in round 1 and appended by it after every round; a scratchpad is fine within one session), the next round's prompt gets the file path, and the round count is inherited from the file, never reset. **Every round must switch lens — never reread the same prompt**: that is one guard against a later round simply following the earlier round's conclusions, the blind start is the other; a same-lens round is a round that never happened. **Parallel multi-vote review (independent votes, none seeing the others, each starting from zero, majority decides) is no longer used**: every vote rediscovers the same problems and none can catch "the prior round's conclusion itself was wrong" — three votes' worth of tokens don't buy three times the coverage; the blind round is not a vote — it only does independent discovery, it counts nothing and rules on nothing (changed 2026-09-22: blind start + chained digging, replacing the 2026-08-12 two-track rule of parallel votes for verdicts and chaining for diagnosis). Diagnosis / root-causing / troubleshooting chain the same way: each round carries the prior round's conclusions and the hypotheses already ruled out, stop after two consecutive rounds with nothing new (same default cap of 4 rounds), and there is no high verdict round (unless the conclusion feeds an irreversible action). In a Workflow script: the blind round runs 2–3 schema-bearing review agents with `await parallel()` inside the review stage and the script merges, dedups and marks conflicts; later rounds that need no fix (conflict settling, verdict) are sequential `await agent()` calls inside the same stage; `parallel()` is used only for the blind round, never to count votes.
3. **Consequence overrides difficulty.** Any unit touching irreversible operations or a large blast radius — data migrations, deleting/overwriting data, auth and permissions, payments, public releases — gets `opus` + `high` on every round of its review chain (3 blind reviewers, dig rounds included) with a mandatory verdict round — a blind round alone may not pass it — no matter how low its implementation tier is, and the review checklist must include "a rollback path exists and is executable". Implementation tiers still follow difficulty; only the verification gets more expensive. Red lines must also be **front-loaded onto the implementation side**: when dispatching implementation agents, write the production red lines into every prompt (see Section 7.4) — don't rely on review as the only backstop. This applies in full to the `mechanical` agent type from Section 2: since it does not auto-load CLAUDE.md, any write-capable mechanical stage must carry the production red lines and any other needed hard rules directly in its own prompt.
4. **Empirical calibrations have a shelf life.** The empirical conclusions in the tier table (opus for frontend, medium as the starting tier, etc.) are dated calibrations that expire when a new model generation ships: rerun a retrospective: rejection rate = count the "review round N" lines in each unit's commit body (`git log --format=%B | grep -c '^评审第'` once rounds are squashed into the unit commit), tokens and wall time from session-report — if medium's rejection rate is <5%, consider probing down to `low`; >30% means the starting tier is too low; when the review model changes generation, reset the rejection-rate baseline and never compare across generations (a stricter reviewer doesn't mean worse implementations). Change the table based on data, not vibes.
...
4. **Worktrees are removed when done**: worktrees may only be created at `.worktrees/<branch-name>` inside the repo (superpowers' default location, which must be in `.gitignore`) or at native EnterWorktree's `.claude/worktrees/` — never at a project-sibling directory (`xxx-wt-*`) or in `/tmp`, even for a throwaway use like merging or auditing. When a merge, an abandoned branch, or a throwaway use ends, clean it up in the same wrap-up batch: `git worktree remove <path>` + `git worktree prune` + delete the local branch once it's merged into main, and say "worktrees cleaned" in the wrap-up report. Whatever this session created, this session cleans, regardless of where it lives — this overrides finishing-a-development-branch's limit of only cleaning up under `.worktrees/`, leaving the rest to the host. Before wrap-up, run `git worktree list` to take inventory and clean anything that isn't part of the batch in progress; anything with uncommitted or unmerged content is not removed — list it and ask the user instead. Basis: a two-week audit of session logs on 2026-09-05 found 88 worktree creations, with every leak located outside `.worktrees/` (where the wrap-up skill doesn't look), leaving nearly 800M of residue in a single project.
5. **Squash review rounds into the unit commit**: a unit's first commit is `feat:`/`fix:` as usual; fixes produced afterward by review rejection, re-review, or final verification are committed with `git commit --squash=<the unit's first commit sha> -m "review round N: <what changed>"` (still push immediately after every commit as before — the backup habit doesn't change). Before merging into main, ahead of the gate decision, the main conversation runs `GIT_EDITOR=true git rebase --autosquash main` on the wip branch to squash each round into its unit commit (the round notes land automatically in the commit body), then `git push --force-with-lease origin <wip branch>`, then merges as usual. `--force-with-lease` is only allowed at this one step, only for its own wip branch — main and shared branches still forbid force push, unchanged, and the implementation agent's red line stands (this step is done by the main conversation at wrap-up, never dispatched). Fixes that come from real-machine/production feedback after a unit is already merged into main still get their own independent `fix:` commit as usual — that's genuine rework and should stay visible in the stats. Review rounds also don't open a new changelog entry in `docs/Progress.md`; they get appended as one line — "review round N: …" — at the end of that unit's existing entry.
...
3. **Write 11 files to disk** (check each one for existing presence first; for any that already exist, list them and ask the user to skip/back up/merge — **never overwrite silently**):
   ...
   - **The eight-doc set**:
     ...
     - `Progress.md` — top half: module status overview table (pending/doing/done); bottom half: changelog (**newest first**; only current-month entries stay in the main file, older ones are archived monthly to `docs/archive/Progress-<YYYY-MM>.md`)
...
### 6c. Auto-sweep merged worktrees (worktree-sweep)

Section 5.4 relies on self-discipline; this hook adds the automation on top: at the end of every turn (`Stop`) and on session exit (`SessionEnd`), `worktree-sweep.sh` runs and checks every non-primary worktree in the current repo's `git worktree list` against five criteria — ① the branch is already merged into main; ② the working tree has no uncommitted changes (including untracked files); ③ none of its git-ignored files belong to the non-regenerable kinds (`.env*`, key/certificate files such as `*.pem`/`*.key`/`id_rsa*`, local databases such as `*.db`/`*.sqlite*`, anything at any depth under `data/` or `secrets/`; the list comes from `git ls-files --others --ignored` file by file with dependency/build directories such as node_modules, .venv and vendor filtered out first; if that command or `git status` fails, or an ignored directory contains a nested git repo, the worktree is treated as "cannot verify" and left alone); ④ it isn't the current session's own directory, no other session has been active in it in the last 30 minutes (judged by the mtime of session-log directories), and no process has its working directory inside it (checked only when `lsof` is available); ⑤ it was neither created nor committed in within the last 30 minutes (judged by the mtime of `index` / `COMMIT_EDITMSG` in the worktree's git admin directory, leaving a window for Workflow `isolation: 'worktree'` subagents to write their first files). If all five hold, it runs `git worktree remove` + `git branch -d` + `git worktree prune` and replies with one line: "worktree-sweep: cleaned up \<path\> (branch \<name\> merged into main, local branch deleted)". When ① holds but ② or ③ fails, it only reminds, never removes (at most once per hour per repo). When ① fails, the branch is treated as in-progress — left untouched, with no reminder either. The worst case for a mis-clean: a directory for a branch already merged into main with no uncommitted changes — the git content can be recreated with `git worktree add`; regenerable ignored files (dependencies, build output) go with the directory, while the non-regenerable kinds listed in ③ block removal.

- **Turning it off**: `touch ~/.claude/.worktree-sweep-off` disables it globally; it exits silently if `jq` is missing or it isn't inside a git repo.
- **Smoke test**: `sh plugins/workflow/hooks/worktree-sweep-smoke-test.sh` (builds a fixture repo in a temp directory, covering: merged-and-clean → removed, merged-but-dirty → reminder only and no repeat within an hour, ignored `.env`-type files → reminder only, ignored files under a `data/` or `secrets/` subdirectory → reminder only, git cannot list ignored files → reminder only, only dependency/build-output ignored files → still removed, not-yet-merged → untouched, current directory → untouched, created or committed in less than 30 minutes ago → untouched, another session active → untouched, a process working inside it → untouched, locked → untouched, kill-switch file → silent, stale registry entries → pruned, non-git directory → silent).
- **Not ported to workflow-codex**: Codex has no hooks mechanism; the Section 5.4 discipline is enforced by convention.
...
3. If isolation is needed, create a worktree first (superpowers:using-git-worktrees, or the Workflow agent's `isolation: 'worktree'`); location and cleanup follow Section 5.4.
4. Workflow-orchestrated implementation: first output a 3-5 line **kickoff summary** (how many units, each unit's model/effort tier, estimated scale) — **don't wait for confirmation, start right away**; the summary just gives the user a visible interrupt window. End the kickoff summary with a ready-to-paste goal command: `/goal "Complete <batch/spec name>" until "<key acceptance criteria from the spec or the ledger items this batch covers> are all satisfied"` — `/goal` is a Claude Code session-scoped built-in command (it auto-evaluates the completion condition every turn, guarding against long sessions drifting off track); it doesn't persist across sessions — persistence across batches relies on the goal ledger. Then break the spec into independent units → parallel implementation agents (`sonnet`, each following TDD, with a self-contained prompt: attach the relevant spec section + the project's CLAUDE.md hard rules + **production red lines and file ownership** — FORBIDDEN FILES (name the files/directories this unit must not touch), never restart shared services, never read or write production data, no force push and no history rewrite that discards changes, touch only the files assigned to this unit) → dispatch a review chain (`opus`, orchestration at the end of this step) to verify and rule on each unit as it completes — besides the implementation, the reviewer must also examine **the tests themselves** (do they cover the spec's acceptance criteria; do they only test the happy path); weak tests count as a rejection, and a rejected unit's re-run splits tests and implementation across two agents → main conversation synthesizes and fixes. Review reports must always **report deltas, not summaries**: only report what's new or overturned relative to the earlier rounds — no "checked it, looks fine" recaps; if there's nothing new, say so explicitly and list the checkpoints that were re-verified. Review orchestration is always blind start + chained digging: each unit first gets 2 review agents run blind in parallel (principle-3 units: 3 agents, every round at high, verdict round mandatory; no vote counting, findings merged into the chain) and passes directly if nothing is at P0/P1 and there is no contradiction; a contradiction alone goes straight to the verdict round; a P1 is fixed and then goes to the verdict round; a P0 is fixed and then gets one dig round (carrying every earlier finding, refuting first and then filling gaps on a new lens) before the verdict round (fixes go through the rejection flow above, then a new run continues the same chain); the last round is the verdict round at `opus` + `high` (see Section 3.2); the prompt for a frontend-visual stage (`opus`) first gives a design direction (cite the hallmark / frontend-design skill), then names the default styles the project's first pass actually produced so the agent avoids them (cream backgrounds, italic accent words in headlines, 01/02/03 section labels, monospace labels, pill buttons and the like), extending the list each round — generic "don't look AI-made" instructions do almost nothing on Opus 5.5, and the rules don't maintain a style list themselves.
5. When this batch touches UI (frontend pages/interactions), run a ui-sweep interaction regression scan before code review (systematically click through every interactive element site-wide), and bring the dead (clicked, no response) / page-error / left-domain list into acceptance review; only rule a real defect after per-item verification in a real browser, and classify false positives (accumulated state, blocking synchronous prompts, buttons that are the current-state control) per the skill's interpretation guide. Skip for backend-only or docs-only batches. After implementation, proceed as usual through superpowers:requesting-code-review → verification-before-completion → finishing-a-development-branch; wherever these skills have a "plan" placeholder (e.g. PLAN_OR_REQUIREMENTS), fill it with the spec path. Once done, update `docs/Progress.md` (status table + changelog) and `docs/PLAN.md` (mark the Phase ✅), and at the same time settle the goal ledger — mark this batch's covered items `done` with evidence (commit/screenshot/spec clause), and register any new goals that surfaced during implementation. If this is the first batch wrap-up of a new calendar month, also archive last month's (and older) changelog entries into `docs/archive/Progress-<YYYY-MM>.md` before wrapping up — the Progress Overview table is never archived, and the archiving happens as its own separate `docs:` commit; before merging this wip branch into main, run the "Squash review rounds into the unit commit" autosquash step from Section 5.5 first. If this batch produced a **reusable building block that other projects could pick up** (a general-purpose middleware, data pipeline, LLM client, deployment template, parser, etc. — not business-specific logic), register it at wrap-up in your organization's component index (the same one §8's Step 0 looks up; if you don't have one, start from a single YAML list with fields like slug/name/capability/repo/path/how_to_integrate/maturity/since/used_by), **verifying the real paths before you write them**, then push the entry back to whatever repo holds that index. This closes the loop with "Step 0: internal first": one looks the index up, this one creates the entries — an index everybody reads and nobody writes decays into a stale list within months.
...
Any requirement, expectation, or complaint the user expresses in conversation — even a single sentence — must be logged into the "goal ledger" in `docs/REQUIREMENTS.md` **the same round** (date + verbatim quote + source), followed by a one-line reply "Logged to the goal ledger." (merged into the same line as the restatement in 7c). When unsure whether something counts as a requirement, log it as `open` anyway — better to over-log than to miss one; **failing to log it counts as not having heard it, and is forbidden.** Requirement-type items from meeting notes and user feedback likewise land in the ledger first and get promoted later; action items (things to do) still go to PLAN/spec, not the ledger.

### 7c. Restate the requirement before starting

Whenever the user raises a requirement (changing logic, adding a feature, fixing a bug, or investigating an issue all count), the first sentence after understanding it must be a one-line restatement: "Got it — next I'll \<one-sentence paraphrase in your own words\>"; number multiple requirements one by one, one sentence each. The restatement merges into the same line as the 7b logging ("Got it — next I'll do X; logged to the goal ledger."), and only after restating does work move on to looking at files and making changes. The restatement is a checkpoint for the user to verify the understanding is correct: the paraphrase must be specific to the action and the object ("fixing the email-sending logic"), not just a bare "Got it"; if it isn't understood, ask directly rather than papering over it with a vague restatement. Example: the user says "change the email-sending logic, specifically…", the reply is "Got it — next I'll fix the email-sending logic."
...
- **Step 0: internal first (a hard step)**: check **your own organization** for reusable building blocks first — a component index, existing projects/modules on internal code hosting (e.g. a self-hosted GitLab); internal hits are presented **side by side** with external open-source candidates for the user to decide — never silently preferred or excluded just for being internal (login-related work is the exception — see the next bullet). If the internal index is unavailable, record that honestly and continue with external research without blocking.
- **If the organization has a shared login library, login work skips research and reuses it**: for any login-related work — sign-in/sign-up, sign-out, password change/reset, sessions and tokens, third-party login, OTP login, auth middleware, login pages, and the like — if the organization already has a shared login library or login service, reuse it unless the user asks otherwise. This bullet is not bound by this section's trigger conditions: it applies to login-related work of any size, and the user saying "no need to research" does not count as asking for a different solution. Treat it as a result Step 0 has already settled: no research, no selection, no side-by-side comparison with external open source; write "reuse <login library>" straight into the spec's "Prior art" section. Implementation only integrates and wires it up — put it in the implementation agent's prompt: never re-implement user tables, password hashing, or token issuance inside the project. If the library lacks a capability, its language or form does not fit the project, or an existing project already has another login solution and might need migrating, ask the user first — never default to building your own. The only exception is the user explicitly asking for a different solution or an in-house build this time, with the reason recorded in the spec; organizations without a login library follow the normal research flow. Reuse does not skip bookkeeping: if the organization keeps a component index, register this project under the login library's used_by once integration is done (the index registration flow of Section 7.5); the integration unit is still reviewed under Section 3.3 (auth is high-risk — "just wiring" does not lower the tier). Developing the login library itself is outside this bullet. Register the library's name and integration steps in the scaffold skill's tech-stack baseline table (`plugins/workflow-en/skills/scaffold/SKILL.md`; Codex edition under `plugins/workflow-codex/`) so new projects carry it from day one.
...
2. Read, in order: `PLAN.md` (roadmap + Phase status + Spec Index) → `Progress.md` (status table + the last 2-3 log entries; older months are archived in `docs/archive/Progress-YYYY-MM.md` — only read those when tracing history) → the latest spec (check against Progress to determine whether it's already implemented) → the last 2-3 entries of `DECISIONS.md` → the action items in the latest section of `MEETINGS.md` → the `open` items in the `REQUIREMENTS.md` goal ledger.
...
└── plugins/
    ...
    │   ├── hooks/                    # auto-scaffold standing rule (hooks.json + inject.sh + auto-scaffold.md) + docs-capture (capture-decisions.sh, signal-reminder.sh, commit-gate.sh, signals-decision.txt, signals-requirement.txt, signals-veto.txt, docs-capture-smoke-test.sh) + worktree-sweep (worktree-sweep.sh, worktree-sweep-smoke-test.sh)
