...
会话跨天、或一个批次/spec 做完,就是收尾点:Claude 主动写一段交接 recap(项目内工作写进 `docs/Progress.md` 变更日志并附下一步;不属于任何项目的散活写 `~/.claude/handoffs/<日期-主题>.md`),然后明确提醒用户开新会话接着干,不在旧会话上续跑下一批次;旧会话里也不再用"继续/好的"式短指令推进新工作。Claude 关不掉用户当前的终端窗口,但交接必须先备好——用户重开 `claude` 即可无缝接上(新会话开局按 §九 走 /whats-next 读 Progress 就能接上)。依据:2026-09-01 一次用量审计,跨天会话只占会话数的 3%,却消耗了全部 token 的 58%;前 100 次大额缓存重建里 98 次发生在跨天会话;超长会话里一条"继续"实测最高消耗 1.22 亿 token。
