...
- `speak-human`:让 Codex 提问与表达遵守说人话纪律——移植自下方「附赠插件:speak-human」的同一套 P1~P9/S1~S6 规则,常驻方式改为写入 `~/.codex/AGENTS.md`(见该 skill 文件末尾「常驻安装」一节的脚本)
...
- **评审链的档位与 Workflow 编排(Claude 侧 0.13.0)是 Claude 专有**:Codex 子代理没有 per-stage 模型/思考档位,也没有 Workflow `agent()`/`parallel()`,"盲审起步 + 接力续挖"在 Codex 侧只保留编排规则(盲审 2 个只读验收 subagent 平行、主对话合并不计票;续挖轮附前面各轮全部发现、先推翻再补漏、换镜头;盲审无 P0/P1 且无矛盾即收,只有矛盾由主对话直接裁定,出 P0/P1 修复后接力到主对话裁定无新 P0/P1 为止,上限 4 轮),裁决不升档、由主对话做,见 AGENTS.md §7.1。
...
| **前端美术** | `opus` 子代理 | 前端视觉/演出/UI 打磨:动画姿势、布局观感、气泡/浮层位置、配色间距这类"看起来对不对"的活(实测 opus 中档比 sonnet 好;派工 prompt 写法见七.4) |
...
| 评审链的盲审轮(首轮 2~3 个互不可见、平行、不计票)与续挖轮(带全部发现,先推翻再补漏);规则 3 单元每轮 high | `opus` | `medium` |
| 评审链的裁决轮(链的最后一轮;盲审无 P0/P1 且无矛盾时可不开;规则 3 单元必跑):终审、验收裁决、安全类评审 | `opus` | `high` |
| 计划与架构设计 | 不派发,留主对话 | — |

上表两条纯机械 stage(定位文件/列清单/盘点;批量迁移/重命名/模板化改码)派发时走 `agent(prompt, { agentType: 'mechanical', model: 'sonnet' 或 'haiku', effort: 'low' })`。这用的是 `workflow` 与 `workflow-en` 插件里自带的 `mechanical` agent 类型,其 frontmatter 设了 `omitClaudeMd: true`,会跳过自动加载的 CLAUDE.md,省 token。只有这两类纯机械 stage 用 `agentType: 'mechanical'`;常规实现(`sonnet`)与所有评审(`opus`)照旧加载 CLAUDE.md。

## 三、升降档四原则

1. **失败才升档,不预付**。实现 stage 一律从 `medium` 起跑;测试不过或被评审打回的单元,重跑时才升 `high`。批量任务里难的通常是少数,全员 high 是拿 90% 的简单题给 10% 的难题买单,而且付的不只是 token,是时延。
2. **`xhigh` / `max` 不进批量 stage**。只留给极少数单点:最难的一次性裁决、安全审计这类"错一次代价很大"的判断。评审想更稳就**加轮次**,不把单轮升到 xhigh,也不数票。评审一律**盲审起步 + 接力续挖 + 裁决收尾**:**盲审轮**——首轮派 2 个(规则 3 的高风险单元 3 个)互不可见的评审 agent 平行跑,各自从零发现,镜头固定为正确性、边界与异常数据(第三个加安全);核对测试本身是每个评审 agent 必做的基础检查,不占镜头位。**不数票**:每个 agent 用同一个 schema 返回 `findings[{file, line, severity: P0|P1|P2, claim, lens}]`,脚本按 file+line 归并(file 用仓库相对路径、line 取起始行、±3 行视为同一位置,无行号的按 file + claim 前 40 字比对;归一化不了的不强行合并,交下一轮去重)、把同一位置结论相反或严重度不同的条目标 `conflict`;语义上的矛盾脚本判不了,交给下一轮裁定。**续挖轮**——之后每轮一个 agent,prompt 显式附前面各轮的全部发现(已确认项、被推翻项、复核过的检查点),任务是先逐条试着推翻、再找漏的,换一个前面没用过的镜头(依次:安全、spec 验收条款覆盖、集成缝合点、回滚与可逆性、对抗反驳),只报新发现与推翻项。**裁决轮**——链的最后一轮(`opus` + `high`,带全链发现出结论,不占镜头);盲审轮与续挖轮都是 `opus` + `medium`。**分流**(盲审合并后):只有 P2 → 判通过,P2 记为遗留;无 P0/P1 但有 conflict → 不修,直接跑裁决轮裁定;有 P1 → 修复后跑裁决轮;有 P0 → 修复后先跑一轮续挖再裁决;裁决轮报出新 P0/P1 → 修复后按同样分流,计入上限。裁决轮无新 P0/P1 且无 P0/P1 未闭合即停(闭合 = 修复后经下一轮复核确认;新 P2 记录即可,不再开轮)。默认上限 4 轮(盲审算一轮),超出要说明原因。**修复与接续**:修复不在评审 stage 里做——一有要修的 P0/P1,该单元本次运行到此为止,主对话按打回流程派实现 agent 修(测试与实现分开两个 agent),修完再开一次运行接原链;链的状态落在仓库内 `docs/reviews/<单元或 spec>-chain.md`(首轮由主对话建,每轮结束后由主对话追加发现与复核点;同一会话内续跑可用 scratchpad),下一轮 prompt 给文件路径,轮次计数由文件继承、不清零。**每轮必须换镜头、不许复读同一个 prompt**——这是防后一轮被前一轮结论带着走的护栏之一,盲审起步是另一道;同镜头续挖等于没开这一轮。**平行多票(各票互不可见、各自从零、数票定结论)不再用于评审**:每票重复找同样的问题,抓不到"上一轮结论本身算错了"级的翻案,三票的 token 换不来三倍覆盖;盲审轮不是投票——它只负责独立发现,不数票、不出结论(2026-09-22 改:盲审起步 + 接力续挖,取代 2026-08-12 的"验收裁决平行多票、诊断链式"双轨)。诊断 / 根因 / 排障同样接力:每轮附上一轮结论与被否决的假设,连续两轮无新发现才停(默认同样上限 4 轮,超出要说明原因),不设 high 裁决轮(除非结论要拿去做不可逆操作)。Workflow 脚本里:盲审轮在评审 stage 内 `await parallel()` 跑 2~3 个带 schema 的评审 agent,脚本合并去重并标 conflict;不需要修复的后续轮(矛盾裁定、裁决)在同一个 stage 内顺序 `await agent()`;`parallel()` 只用于盲审,不做投票计数。
3. **后果覆盖难度**。凡触及不可逆操作或高爆炸半径的单元——数据迁移、删除/覆写数据、鉴权与权限、支付、对外发布——无论实现档位多低,评审链每一轮(盲审 3 个、续挖轮也含)都用 `opus` + `high`,裁决轮必跑、不许盲审直接判通过,评审检查项必须包含"回滚路径存在且可执行"。实现档位照旧按难度走,贵的只是验证。红线同样要**前置到实现侧**:派实现 agent 时就把生产红线写进每个 prompt(见七.4),不能只靠评审兜底。这条对二里的 `mechanical` agent 类型同样全额适用:它不自动加载 CLAUDE.md,任何有写权限的机械 stage 必须把生产红线与其他必需硬规则直接写进自己的 prompt。
4. **经验校准有保质期**。档位表里的经验性结论(opus 前端、medium 起跑等)均视为有日期的校准,主力模型换代后失效待验:复盘一次:打回率用 git 数各单元正文里的「评审第 N 轮」行(压进单元提交后 `git log --format=%B | grep -c '^评审第'`),token 与耗时用 session-report——medium 起跑的打回率 <5% 考虑下探 `low`,>30% 说明起跑档太低;评审模型换代时打回率基线清零重采,不跨代比较(评审变严不等于实现变差)。凭数据改表,不凭手感。
...
Workflow 工具单次运行的并发 agent 数上限默认约为 `min(16, 核数 − 2)`。环境变量 `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`(合法范围 1–256)可以调高这个上限。这里只是记录这个开关的存在——不改任何默认值。只在机器有富余 CPU 和内存余量时才调高;在已经吃紧的机器上调高会招致过载。
...
4. Workflow 编排实现:先输出 3~5 行**开工摘要**(拆了几个单元、各自 model/effort 档位、预估规模),**不等待确认直接开跑**——摘要只是给用户一个看得见的打断窗口。开工摘要末尾附一行现成可贴的目标命令:`/goal "完成 <批次/spec 名>" until "<spec 验收条款要点或本批覆盖的台账条目>全部满足"`——`/goal` 是 Claude Code 会话级内置命令(每轮自动评估完成条件,防长会话做着做着跑偏),不跨会话,跨批次的持久性靠目标台账。随后按 spec 拆独立单元 → 并行实现 agent(`sonnet`,每个遵守 TDD,prompt 自包含:附 spec 相关段落 + 项目 CLAUDE.md 硬规则 + **生产红线与文件所有权**——FORBIDDEN FILES(本单元不许碰的文件/目录点名列出)、绝不重启共享服务、绝不读写生产数据、禁止 force push 与任何丢弃改动的历史改写、只改分给本单元的文件)→ 每单元完成即派评审链(`opus`,编排见本步末尾)验证裁决,评审除核对实现外必须核对**测试本身**(是否覆盖 spec 对应验收条款、是否只测 happy path),测试弱视同打回,被打回的单元重跑时测试与实现分开派两个 agent → 主对话汇总修复。评审报告一律**报差异不报摘要**:只报与前面各轮不同的新发现与推翻项,禁止"检查了一遍没问题"式复述;无新发现就写明"无新发现"并列出复核过的检查点。评审编排一律盲审起步 + 接力续挖:每单元先派 2 个互不可见的评审 agent 平行盲审(规则 3 单元 3 个、每轮 high、裁决轮必跑;不计票,发现合并进链),无 P0/P1 且无矛盾直接判通过;只有矛盾就直接裁决;有 P1 修复后直接裁决;有 P0 修复后先续挖一轮(带前面各轮全部发现、先推翻再补漏、换镜头)再裁决(修复走上面的打回流程,修完再开一次运行接原链);末轮裁决 `opus` + `high`(见三.2);前端视觉 stage(`opus`)的 prompt 先给设计方向(引用 hallmark / frontend-design skill),再点名本项目首轮产出里出现过的默认样式让它避开(米白背景、标题斜体点缀词、01/02/03 编号标签、等宽字体标签、胶囊按钮之类),逐轮补清单——"别像 AI 做的"这类泛指令对 Opus 5.5 基本无效,规则里不自己维护样式清单。
...
- **第 0 步:内部先行(硬步骤)**:先查**自己组织内部**有没有现成可复用件——组件索引、内部代码托管(如自建 GitLab)上的既有项目/模块;内部命中的候选与外部开源候选**同台呈现**给用户拍板,不得因为"是内部的"就默认优先或默认排除(登录相关是例外,见下条)。内部索引不可用时如实记录后继续外部调研,不阻塞。
- **组织内已有通用登录库时,登录相关不调研、直接复用**:凡与登录相关的工作——登录/注册、登出、改密/找回密码、会话与令牌、第三方登录、验证码登录、鉴权中间件、登录页等——组织内已有通用登录库或登录服务的,用户没有特殊要求就直接复用它。本条不受本节触发条件限制:任何规模的登录相关工作都适用,用户说"不用调研"不算要求换方案。这是第 0 步已经定下的结果:不调研、不选型、不与外部开源一起比较,spec 的「Prior art」直接写「复用 <登录库>」;实现只做接入,派实现 agent 时写进 prompt:不在项目里重写用户表、密码哈希、令牌签发;该库缺的能力、语言或形态与项目不匹配、存量项目已有别的登录方案要不要迁,都先问用户,不默认自建。只有用户本次明确要求换方案或自研时例外,例外原因写进 spec;组织内没有登录库的照常走调研。复用之后照样要登记:组织有组件索引的,接入完成后把本项目登记进该登录库的 used_by(§七.5 的索引登记流程);接入单元的评审仍按三.3 走(鉴权类高风险,不因"只是接入"降档)。登录库自身的开发不适用本条。登录库的名字与接入方式登记进 scaffold skill 的技术栈基线表(`plugins/workflow/skills/scaffold/SKILL.md`;Codex 版在 `plugins/workflow-codex/`),新项目铺设时就带上。
