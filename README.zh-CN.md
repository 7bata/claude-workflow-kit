...
- **跨账号识别 + 文件交接降级**:跨会话发现按登录账号隔离(每个会话只在自己 profile 的注册表登记,双向实测定论)——另一个账号开的窗口既看不见也发不过去。零命中时 skill 会查共享 socket 目录里的多余 socket 线索,说明大概率是跨账号窗口,并降级为自包含交接文件 + 一行粘贴指令,由用户亲手贴进目标窗口。
