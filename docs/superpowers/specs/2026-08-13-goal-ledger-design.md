...
- 依据:stella 会话交接的活案例——「代表分组收敛为领域唯一」2026-07-27 口头提出、未落任何文档、三周蒸发,2026-08-12 验收才审计出来;「手工新建项目入口」做了但不在用户动线,用户以为没做。根因①对话需求无持久落点;②多批次推进无对账清单。另:`/goal` 为 Claude Code 内置命令(设完成条件、每轮 Haiku 评估、达成自动清除),**会话级不跨会话**(官方文档 code.claude.com/docs/en/goal.md 查证)——故只能作会话内粘合层,治蒸发必须靠持久台账。
