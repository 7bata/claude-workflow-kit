...
- **定案:高召回软提醒(2026-08-14 五轮后 Tony 拍板)**:验收标准从「任意对抗句零误报」
  改为「套内判例全绿 + 软件会话领域内误报可忽略」。第五轮 opus/high 实证:召回集外 22/22 满分;
  剩余误报全部来自故意跳出软件语境的生活句(家装/体育/厨房),其枚举宾语(功能/入口/表格)
  在生活语境天然成立——词表法边界,不再追。提醒语自带免疫说明(「若只是日常表述,忽略本提示」),
  收信人是有判断力的模型,非拦截。已知残留(记录不修):冒号引导头子句的压制不回溯、
  纯空格分隔的多子句整条压制、commit-gate 对 heredoc 正文与引号内分号的残留误触发。
