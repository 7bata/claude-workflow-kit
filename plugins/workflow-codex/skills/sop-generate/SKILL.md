...
- **建议前置**:若已安装 `ui-sweep` skill,批量截图前先跑一遍全站交互遍历(把每屏可交互元素系统性点一遍),能提前揪出交互异常,避免截图记录了带缺陷的界面状态而不自知;该引擎是普通 node 脚本(`sweep.mjs`),不依赖 Claude Code 专属能力,Codex 侧同样可用。未安装则跳过此步,不影响后续流程。
