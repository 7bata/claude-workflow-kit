...
```bash
# 1. 主路径:直接用本 skill 自带的 crawl.mjs 采集(登录→遍历→截图三段式)。
#    凭据经环境变量传入,不要写进本文件、不要写进 --user/--pass 命令行参数
#    (会落进 shell history 与 ps aux):
...
# 2. 关键操作(提交表单、导出报表等)的前/后两张截图,可选传入动作清单:
node <本 skill base directory>/scripts/crawl.mjs \
  --url <部署URL> --out docs/sop-images --actions <动作清单.json>

# 3. 若当前环境已配置浏览器类 MCP,可改用其工具做导航/快照/截图代替以上两步,
#    产出物约束(截图 + accessibility 摘要、不整页喂模型)不变。
```
