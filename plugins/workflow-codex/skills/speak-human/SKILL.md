...
1. 只在内心推演(reasoning,内部思考过程)里想过——思考过程用户默认读不到,
   界面上也随手折叠,它不属于对话;
2. 散落在之前几轮里——问答轮答完就滚出屏幕,普通正文也早被后续输出顶上去;
   不在本条回复里,就当用户看不见;
...
| 提问的开头一句 | 一句:要你定什么;背景按 P2 三交代另起,不塞进这一句 |
...
8. 拍板对象的内容或骨架,就在本条回复正文或提问自身里吗?(内心推演过、
   之前几轮问过、写进过文件,都不算)(P9)
...
2. 提取规则正文:去掉文件顶部的 YAML frontmatter(第一个 `---` 到第二个 `---`
   之间那一段),也去掉本「常驻安装」这一节本身——只留 P1~P9 / S1~S4 / 自检
   清单 / 例外条款 / 出处。
