#!/usr/bin/env node
/**
 * sop-generate 降级脚本 — 原生 Playwright 版
 *
 * 仅在 playwright-mcp 未配置时使用(`claude mcp list | grep -i playwright` 无结果)。
 * 三段式:登录 → 遍历 → 截图。脚本只产出截图文件 + 每页的 accessibility 摘要 JSON,
 * 不写文案 —— 文案由 Claude 读摘要 JSON 生成,严禁把整页截图喂给模型。
 *
 ...
 *   面依然存在。调用方(Claude)必须优先用环境变量方式调用本脚本。
 *
 ...
 *   记进 <out>/_actions/_downloads.json,供 Claude 写"操作确有其效"的证据(文件名/字节数),
